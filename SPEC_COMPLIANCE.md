...
| # | Requirement | Implementation | Test | Status |
|---|-------------|----------------|------|--------|
| 3.1 | Use QRC (and ECP when appropriate) | QRC: `src/QscDspDevices/Protocol/QrcFramer.cs`, `JsonRpcDispatcher.cs`, JsonRpc/*. ECP: `src/QscDspDevices/Protocol/Ecp/*` (framer, quoting, command builder, response parser, dispatcher) + `src/QscDspDevices/Connectivity/Ecp/*` (queue, connection manager, hydrate, autopoll, engine-state probe, redundant pair, routing facade) + `src/QscDspDevices/AudioControl/Ecp/*` + `src/QscDspDevices/LogicTriggers/Ecp/*`. Selection by well-known port: 1710 → QRC, 1702 → ECP. M-ECP-part-3 added AutoPoll-based cache reconciliation and `sg`-poll redundancy on the M6 `RedundantConnectionPair` shape (renamed `EcpRedundantConnectionPair` for ECP). | QRC: 40+ unit + integration. ECP: 130+ unit (framer/quoting/commands/responses/dispatcher/queue/connection-manager/hydrate/autopoll/engine-state-probe/services/redundant-pair) + integration end-to-end (single-core happy paths + redundancy failover) | ✅ QRC + ECP at full feature parity for the framework surface (single-core + redundant) |
