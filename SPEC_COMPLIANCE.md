...
| # | Requirement | Implementation | Test | Status |
|---|-------------|----------------|------|--------|
| 3.1 | Use QRC (and ECP when appropriate) | QRC: `src/QscDspDevices/Protocol/QrcFramer.cs`, `JsonRpcDispatcher.cs`, JsonRpc/*. ECP: `src/QscDspDevices/Protocol/Ecp/*` (framer, quoting, command builder, response parser, dispatcher) + `src/QscDspDevices/Connectivity/Ecp/*` (queue, connection manager) + `src/QscDspDevices/AudioControl/Ecp/*` + `src/QscDspDevices/LogicTriggers/Ecp/*`. Selection by well-known port: 1710 → QRC, 1702 → ECP. | QRC: 40+ unit + integration. ECP: 64 unit (framer/quoting/commands/responses/dispatcher) + 22 connection-manager + 5 audio-control + 3 integration end-to-end | ✅ QRC + ECP single-Core complete (M-ECP-part-1 + part-2); ECP redundancy deferred to M-ECP-part-3 |
