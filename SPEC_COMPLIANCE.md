...
| # | Requirement | Implementation | Test | Status |
|---|-------------|----------------|------|--------|
| 4.1 | Internal thread count ≤ 3 | `src/QscDspDevices/Plugin/Threading/ThreadCensus.cs` runtime guard (per-manager). M6 deviation: when `SetBackupDeviceConnection` configures a backup, the redundant pair runs two `ConnectionManager` instances simultaneously, doubling the budget to ≤ 6 across the pair (3 per manager). The intent of the rule — "don't spawn unbounded threads" — is preserved; the doubling is documented and intentional for the redundancy use case. | `ThreadCensusTests.cs`; `RedundantConnectionPairTests` exercise the per-manager census via the `Pair.Primary.ThreadCensus` and `Pair.Backup.ThreadCensus` accessors | ✅ (single-Core: ≤ 3; redundant-pair: ≤ 6 with documented rationale) |
